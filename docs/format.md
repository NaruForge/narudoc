...
CLI의 문서 크기 한도는 BOM을 포함한 UTF-8 기준 10 MiB다. 새 문서와 편집 결과에도 같은 한도를 적용하며, 초과하면 `NARU_LIMIT`로 실패하고 원본을 보존한다. `--dry-run`도 저장과 같은 검사를 수행한다. 변환된 HTML 출력에는 이 문서 크기 한도를 적용하지 않는다.
