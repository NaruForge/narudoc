...
type Bindings = { [K in OperationName]: Binding<K> | null };
...
export function checkBindings(definitions: Record<string, { input: InputSchema }>, bindings: Record<string, { command: string; fields: Record<string, unknown> } | null>): void {
  ...
  for (const [name, definition] of Object.entries(definitions)) {
    ...
    if (!binding) throw Error(`${name}: missing CLI exposure choice.`);
...
export function operationExample(name: OperationName, bindings: Bindings = operationBindings) {
  const binding = bindings[name], example = operationDefinitions[name].example as Record<string, unknown>;
  if (binding === null) return {
    args: ['batch', 'practice.narudoc', '--operations', 'input.json', '--revision', 'SHA256'],
    json: { schemaVersion: 1, operations: [{ type: name, ...example }] } as Record<string, unknown>,
    command: 'narudoc batch practice.narudoc --operations input.json --revision SHA256 --dry-run --json',
  };
...
export function createCommands(bindings: Bindings = operationBindings): Record<string, Command> {
  checkBindings(operationDefinitions, bindings);
  const commands = { ...hostCommands };
  for (const name of Object.keys(operationDefinitions) as OperationName[]) {
    const definition = operationDefinitions[name], binding = bindings[name];
    if (binding === null) continue;
    ...
    commands[binding.command] = { operation: name, description: definition.description, options: { ...options, ...writes }, example: operationExample(name, bindings).command, file: true };
  }
  return commands;
}
...
export async function bindOperation(command: string, values: Record<string, string | boolean | undefined>, readJson: (file: string) => Promise<unknown>): Promise<Operation> {
  ...
  if (binding === null) throw new NaruError('NARU_ARGUMENT', 'Use the batch command for this operation.');
...
export function capabilities(name?: string, bindings: Bindings = operationBindings) {
  if (!name) return { schemaVersion: 1, operations: Object.entries(operationDefinitions).map(([type, value]) => ({ type, description: value.description, advanced: !!('advanced' in value && value.advanced), command: bindings[type as OperationName]?.command ?? 'batch' })) };
  ...
  return { schemaVersion: 1, operation: name, description: definition.description, input: operationSchema(key), example: { type: name, ...definition.example }, target: definition.target, effect: definition.effect, retry: definition.retry, cli: operationExample(key, bindings) };
}
export function commandHelp(positionals: string[] = [], catalog = commands): string {
  const two = positionals.slice(0, 2).join(' '), one = positionals[0] ?? '';
  const command = Object.hasOwn(catalog, two) ? two : Object.hasOwn(catalog, one) ? one : undefined;
  if (!command) {
    const listed = Object.entries(catalog).filter(([name]) => !one || name.startsWith(one + ' '));
...
export function commandReference(catalog = commands): string {
  return '| Command | Options (`!` required) | Purpose |\n| --- | --- | --- |\n' + Object.entries(catalog).map(([name, spec]) => `| \`${name}${spec.file ? ' FILE' : ''}\` | ${Object.entries(spec.options).map(([flag, opt]) => `\`--${flag}${opt.required ? ' !' : ''}\``).join(', ')} | ${spec.description} |`).join('\n');
}
